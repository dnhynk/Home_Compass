...
def feature_story(profile: dict[str, object], evidence_dir: Path):
    ...
    features = [
        ...
        ["F6", "AI 상담", "엔진 도구 호출 후 결과를 설명하는 대화형 상담, 라이브 제공자 실패 시 템플릿 폴백", "AI 상담 카드", "완료"],
    ...
    story += section_block("3", "사용자 이용 흐름*", [
        ...
        "<b>5) AI 질문:</b> 하단 AI 상담의 추천 질문을 누르거나 자연어로 질문합니다. 질문에 필요한 계산 도구를 호출해 답합니다. AI 연결을 이용할 수 없으면 기본 안내로 전환되며 그 상태를 표시합니다.",
        staff_flow,
    ])
    ...
    story += section_block("5", "MVP 검증 방법*", [
        ...
        bullet("<b>절차 C - AI 상담:</b> '전세랑 월세 중 뭐가 나아?' 버튼을 누릅니다. 답변이 진단 결과의 시나리오와 상한을 설명하며, AI 응답이 질문에 답하는지 확인합니다. 연결 실패 시에는 기본 안내 표시와 답변을 확인합니다."),
    ...
    story += evidence_appendix("부록 A. 검증 기준 화면", [
        ...
        (evidence_dir / "home_compass_chat.png", "[그림 3] AI 상담 - 전월세 비교 도구의 결과를 바탕으로 질문에 답변"),
    ])
    return story
