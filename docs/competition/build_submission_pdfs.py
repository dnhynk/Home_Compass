...
def humanize_markup(text: str) -> str:
    replacements = {
        ...
        "<b>절차 A - 핵심 진단:</b> URL 접속 → 위 표의 샘플 입력값과 희망지역 서울 마포구 입력 → '주거비 진단 시작' → 위 표의 결과와 화면 수치를 대조합니다.":
            "<b>절차 A · 핵심 진단:</b> URL에 접속해 위 표의 샘플 입력값과 희망지역 서울 마포구를 입력합니다. 이어서 '주거비 진단 시작'을 실행해 위 표와 화면 수치를 대조합니다.",
    ...
    text = text.replace(" → ", ", 이어서 ")
    closing = text.find("</b>")
    if text.startswith("<b>") and closing >= 0 and text[closing + 4:].startswith(" "):
        label = text[3:closing].rstrip()
        if not label.endswith(":"):
            raise ValueError(f"inline bold label is missing a colon: {label}")
    return text
