...
# --------------------------------------------------------------------------
#
# `firsthome.main` 을 import 하는 것이 곧 기동이다. 모듈 최상단에서 모델 상수 전수
# 존재를 검증하고, 컷오버 후 그 출처는 저장소다. 시드된 저장소가 없으면
# `tests/test_engines.py` 는 테스트가 한 건도 돌기 전에 collection 단계에서 죽는다.
#
# **개발자 로컬 DB(`backend/var/firsthome.db`)에 기대지 않는다.** 그것이 있으면 통과하고
...
# `firsthome.main` 을 import 하므로 세션 스코프 픽스처는 이미 늦다.
#
...
_STORE_DIR = tempfile.mkdtemp(prefix="firsthome-tests-")
atexit.register(shutil.rmtree, _STORE_DIR, ignore_errors=True)
os.environ["FIRSTHOME_STORE_URL"] = f"sqlite://{os.path.join(_STORE_DIR, 'session.db')}"

# --------------------------------------------------------------------------
# 4단계 — 시드 계정 비밀번호를 **환경변수로** 주입한다 (SPEC 6.3 · 부록 A)
# --------------------------------------------------------------------------
#
# `firsthome.main` 을 import 하면 기동 훅이 상담원·규칙관리자 계정을 만든다. 환경변수가
...
os.environ.setdefault("FIRSTHOME_SEED_COUNSELOR_PASSWORD", secrets.token_urlsafe(16))
os.environ.setdefault("FIRSTHOME_SEED_RULE_MANAGER_PASSWORD", secrets.token_urlsafe(16))
