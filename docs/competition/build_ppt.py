...
def slide_08_e1(prs):
    ...
    # ★ 둘째 줄에 붙어 있던 `(safe)` 는 엔진이 더는 내지 않는다 — 원시 enum 이 시민
    #   화면에 새던 것을 걷어냈기 때문이다. 덱만 그것을 들고 있었다.
...
def slide_10_e3(prs):
    ...
    # POST /api/analyze 의 scenarios[].rationale 에서 **그대로** 가져왔다.
    # 첫 줄은 회귀 프로필 baseline 의 두 번째 시나리오, 둘째 줄은 non_metro 의 것이다.
    # ★ 예전 첫 줄은 있지도 않은 8,000만원 사례를 인용했고 **실제 문장의 둘째 절을
    #   잘라냈다.** 둘째 줄은 엔진이 더는 붙이지 않는 `(stretch)` 를 달고 있었다.
    #   두 결함 다 `test_competition_deck` 의 괄호 enum 파수병이 이제 막는다.
    _rationale_box(slide, 5.30,
                   ["보증금에 묶이는 자기자본 5,500만원의 기회비용을 연 3.3%로 계산해 897만원을 총비용에 포함했습니다."
                    " 보증금 자체는 계약 종료 시 돌려받으므로 비용이 아닙니다.",
                    "월 환산비용이 권장액 73만원은 넘지만 상한 86만원 이내여서 무리하면 가능한 구간입니다."])
