...
WORKER_PATHS: dict[str, list[str]] = {
    ...
    #
    # --- 이 Run 의 워커 배정은 전부 회수했다 (run_b70307c8c66d) -----------------
    #
    ...
    #
    # --- 심사 계정 결정의 파급 (run_b70307c8c66d) — **과업이 닫히면 지운다** --------
    #
    # 사용자가 심사위원용 직원 계정을 제출물에 넣기로 했다. 그래서 상담원·규칙관리자
    # 화면이 심사 대상이 됐고, 「심사 범위 밖」이라 미뤄 둔 결함이 실제 장애가 됐다 —
    # `styles.css` 가 620px 이하에서 로그인 칸을 `display: none` 으로 지운다. 즉
    # **휴대폰에서 상담원 로그인이 불가능하다.** `/admin/` 은 자체 게이트가 있어
    # F8-F10 은 닿지만 F7 은 시민 화면 상단바가 유일한 경로다.
    "W-staff-login": OWNERSHIP["web"],
}
