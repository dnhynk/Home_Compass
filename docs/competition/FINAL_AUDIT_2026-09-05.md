...
발급된 HTTPS 주소의 제출 전 검사 **15 PASS, 0 PENDING, 0 FAIL**, 상담원·관리자 계정의
...
| 입력 정보 안내 | 서버로 프로필을 전송하면서 입력값이 브라우저를 벗어나지 않는다고 표시 | 진단·AI 상담을 위한 서버 전송을 정확히 안내 |
...
7. 교체 후 같은 상담원·관리자 계정 비밀번호로 로그인과 세션 확인 성공.
...
- 상담원·관리자 계정의 서로 다른 32자 비밀번호를 생성하고 운영 컨테이너에서 검증했다.
- 같은 값을 로컬 프로필의 관리자 계정 안내와 실제 PC 운영 환경에 반영했다. 공개 HTTPS 로그인으로 일치를 확인했다.
- `build_submission_pdfs.py --strict`로 기획서 4쪽·기능 명세서 5쪽을 재생성했다.
- Poppler로 9쪽을 렌더링해 이름, 계정 안내, 표, 페이지 구분, 증거 이미지 배치를 확인했다.
- 기존 PDF 증거 스크린샷은 09-04 촬영본이다. 이후 서버 전송 안내 문구를 바로잡았으며 진단·상담 화면 구성과 계산 결과는 동일하다.

## 실제 무료 배포

- 서버 임대료가 없는 사용자 PC의 Python 서버 + Tailscale Funnel을 선택했다. 유료 Render 서비스는 생성하지 않았다. OpenAI API 이용료는 별도다.
- 검증한 Git 커밋의 별도 복사본을 운영한다. 실제 기준 커밋은 로컬 최종 상태 문서에 기록한다. 원본 작업 디렉터리를 수정해도 서버 코드는 바뀌지 않는다.
...
- 서버 프로세스 트리를 강제 종료한 뒤 21.37초 내 자동 복구됐다. 감독 프로세스 재시작을 포함해 11개 테이블과 상담원·관리자 계정이 보존됐다. 백업 무결성도 통과했다.
- 공개 HTTPS에서 익명 홈페이지·진단·Luna 라이브 상담, 상담원·관리자 계정의 후속 세션을 확인했다. 관리자 API의 익명 접근과 비밀 파일 경로는 차단됐다.
